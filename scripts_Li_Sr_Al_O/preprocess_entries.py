...
#Merging duplicate entries of ICDD
icdd_entries = precess.entries
precess = ICDDEntryPreprocessor(deepcopy(icdd_entries), chemsys, oxide_system)
precess.theta_to_q()
precess.get_xrd()
precess.merge_by_polymorph(bin_number=1000, gaussian_filter=4, R_cutoff=0.2, plot=False)
...
precess.merge_by_polymorph(bin_number=1000, gaussian_filter=4, R_cutoff=0.2, plot=False)
print('[ICDD and ICSD] after merging XRD-polymorph entries', len(precess.entries))

#Merging XRD-group entries of ICSD and ICDD
groups = precess.merge_by_xrd(bin_number=1000, gaussian_filter=4, R1_cutoff=0.22, R2_cutoff=0.3, plot=False)
...
#Save the null_stability, stable, unstable entries
